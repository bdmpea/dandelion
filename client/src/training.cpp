...
void training::set_desk(cards_for_training* new_desk) { //TODO: receive cards from user's db
...
void training::update_card_appearance() {
    if (state == question) {
        ui->CardWidget->setStyleSheet(question_appearance);
        ui->PromptLabel->setText(current_card->get_word());
    }
    else {
        ui->CardWidget->setStyleSheet(answer_appearance);
